...
export const RewardsInjectorTable: React.FC<RewardsInjectorTableProps> = ({ data, tokenSymbol }) => {
    ...
    const SortableHeader: React.FC<{ column: keyof RewardsInjectorData; label: string }> = ({ column, label }) => (
        <Flex
            alignItems="center"
            cursor="pointer"
                ...
                <Tbody>
                    {sortedData.map((row, index) => (
                        <Tr key={index}>
                            <Td>
                                <Tooltip label={row.gaugeAddress}>
                                    <Text isTruncated maxWidth="200px">{row.gaugeAddress}</Text>
                                </Tooltip>
                            </Td>
                            <Td>{row.poolName}</Td>
