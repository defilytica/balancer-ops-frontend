...
export const ConfigurationCard: React.FC<ConfigurationCardProps> = ({
  ...
  const getChartTokens = (): PoolCompositionChartToken[] => {
    return config.tokens
      .filter(
        token =>
          token.symbol &&
          (config.type === "weighted" ? token.weight !== undefined && token.weight > 0 : true),
      )
      .map(token => ({
        symbol: token.symbol,
  ...
  const generatePoolSymbol = (): string => {
    ...
    if (config.type === "weighted") {
      ...
      poolName = validTokens.map(token => token.symbol.toUpperCase()).join("/");
    }
  ...
  useEffect(() => {
    ...
    const validTokens = config.tokens.filter(
      token => token.symbol && (config.type === "weighted" ? token.weight !== undefined : true),
    );

    if (validTokens.length >= 2) {
      // Only generate name with 2+ tokens
              ...
              <>
                <Tr>
                  <Td fontWeight="medium">
                    {" "}
                    <Text fontSize="sm">Pool Name </Text>
                  </Td>
                ...
                <Tr>
                  <Td fontWeight="medium">
                    {" "}
                    <Text fontSize="sm">Swap Fee </Text>
                  </Td>
                ...
                {config.settings && (
                  <>
                    <Tr>
                      <Td fontWeight="medium">
                        {" "}
                        <Text fontSize="sm">Fee Management </Text>
                      </Td>
                      <Td>
                        <Tag
                          size="sm"
                          colorScheme={
                            config.settings.weightedSpecific?.feeManagement ||
                            config.settings.stableSpecific?.feeManagement
                              ? "green"
                              : "red"
                          }
                        >
                          {config.type === "weighted"
                            ...
                            : config.settings.stableSpecific?.feeManagement.type}
                        </Tag>
                      </Td>
                    </Tr>
                    <Tr>
                ...
                {config.type === "composableStable" && config.settings.stableSpecific && (
                  <>
                    <Tr>
                      <Td fontWeight="medium">
                        <Text fontSize="sm">Amplification Parameter</Text>
                      </Td>
  ...
  return (
    <Card variant="outlined" size="sm">
      <CardHeader py={3}>
        <Heading variant="special" size="md">
          Pool Summary
        </Heading>
      </CardHeader>
      <CardBody>
