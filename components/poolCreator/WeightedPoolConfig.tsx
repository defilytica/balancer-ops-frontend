import { Button, Stack, useToast, HStack } from "@chakra-ui/react";
...
export const WeightedPoolConfig = ({
  config,
  onConfigUpdate,
  skipCreate,
}: WeightedPoolConfigProps) => {
  const { chain } = useAccount();
  const [tokens, setTokens] = useState<TokenWithBalance[]>(
    config.tokens.length
      ? config.tokens
      : [
          { address: "", weight: 0, symbol: "", amount: "", locked: false },
          { address: "", weight: 0, symbol: "", amount: "", locked: false },
        ],
  );
  ...
  useEffect(() => {
    if (priceData?.tokenGetCurrentPrices) {
      setTokens(currentTokens => {
        ...
        const pricesChanged = updatedTokens.some(
          (token, index) => token.price !== currentTokens[index].price,
        );
  ...
  const updateWeight = (index: number, newWeight: number) => {
    ...
    const adjustableTokens = tokens.filter((t, i) => i !== index && !t.locked);

    const lockedWeight = tokens.reduce(
      (sum, t, i) => (i !== index && t.locked ? sum + (t.weight || 0) : sum),
      0,
    );
    ...
    <Stack spacing={6}>
      ...
      <HStack spacing={4}>
        ...
        <Button
          ...
          isDisabled={
            tokens.length < 2 ||
            !tokens.every(t => t.price && t.weight) ||
            tokens.reduce((sum, t) => sum + t.weight, 0) !== 100
          }
        >
          Optimize Amounts
        </Button>
