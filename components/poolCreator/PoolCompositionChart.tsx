...
export const PoolCompositionChart = ({ tokens }: PoolCompositionChartProps) => {
  ...
  return (
    <Box borderWidth="1px" borderRadius="md" borderColor={borderColor} p={2} w="full">
      <Center>
        <Grid templateColumns={{ base: "1fr", sm: "160px 1fr" }} gap={4} maxW="fit-content">
          <GridItem>
                ...
                {data.map((entry, index) => (
                  <ListItem key={entry.name}>
                    <Flex align="center" fontSize="xs" minW="140px">
                      {entry.logoURI ? (
                        <Avatar src={entry.logoURI} size="xs" name={entry.name} bg={entry.color} />
                      ) : (
