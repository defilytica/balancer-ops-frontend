...
const PoolLookup = ({ onPoolFound }: { onPoolFound: (poolData: PoolConfig) => void }) => {
  ...
  const fetchPoolData = async (poolAddress: string): Promise<PoolConfig> => {
    try {
      ...
      if (!contractCode) {
        throw new Error(
          `No contract found at address ${poolAddress} on ${chain?.name || "current"} network. Please verify the address and network.`,
        );
      }
      ...
      const totalSupply = await publicClient
        ...
        .catch(error => {
          if ((error as Error).message.includes("Contract read returned invalid data")) {
            throw new Error(
              `The contract at ${poolAddress} is not a valid Balancer pool contract. Please verify you're using the correct pool address.`,
            );
          }
          throw error;
        });
      ...
      if (totalSupply !== BigInt(0)) {
        throw new Error(
          "Pool is already initialized, please provide liquidity through the official Balancer website.",
        );
      }
      ...
      specificSettings: {
        feeManagement: {
          ...
          type:
            ...
            "0xBA1BA1ba1BA1bA1bA1Ba1BA1ba1BA1bA1ba1ba1B".toLowerCase()
              ? "governance"
              : "fixed",
        },
      },
  ...
  const fetchComposableStablePoolData = async (
    poolAddress: string,
    poolABI: any,
    tokens: string[],
  ) => {
    if (!publicClient) throw new Error("Public client not initialized");
    const [amplificationParameter, swapFee, name, symbol, owner, yieldFeeExempt, rateProviders] =
      await Promise.all([
        publicClient.readContract({
          address: getAddress(poolAddress),
          abi: poolABI,
    ...
    return {
      ...
      specificSettings: {
        amplificationParameter:
          Number(amplificationParameter[0]) / Number(amplificationParameter[2]),
  ...
  const fetchTokenDetails = async (
    ...
    return {
      ...
      logoURI:
        tokensData?.tokenGetTokens.find(token => token.address === tokenAddress.toLowerCase())
          ?.logoURI || "",
    };
  };
