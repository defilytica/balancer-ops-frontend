...
export function getSurgeFeePercentage(
  ...
  const surging = isSurging(surgeThresholdPercentage, poolCurrentBalances, newImbalance);
  ...
  const newFee =
    staticFeePercentage +
    ((maxSurgeFeePercentage - staticFeePercentage) * (newImbalance - surgeThresholdPercentage)) /
...
export function isSurging(
  ...
  return newTotalImbalance > currentImbalance && newTotalImbalance > surgeThresholdPercentage;
}
