...
export const fetchChainlinkData = async (): Promise<ChainlinkData[]> => {
    const response = await fetch('https://raw.githubusercontent.com/BalancerMaxis/multisig-ops/upkeeps/upkeeps.csv');
        ...
        Papa.parse(csvText, {
            ...
            complete: (results: { data: any[]; }) => {
                ...
                processedData.forEach(row => {
                    row.estimated_actions_left = row.link_per_perform > 0
