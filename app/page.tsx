    ...
    >
      <Container maxW="container.xl">
        <VStack spacing={8} textAlign="center">
          <Heading as="h1" p={3} size="4xl" variant="special" fontWeight="bold">
            Welcome to Balancer Ops Tooling
          </Heading>
          <Text fontSize="xl" variant="secondary">
            Streamlining Balancer DAO operations with our powerful suite of
            tools
          </Text>
          {!showOptions ? (
            <Button
              ...
              {options.map((option, index) => (
                <GridItem key={index}>
                  <Link href={option.href} _hover={{ textDecoration: "none" }}>
                    <Button size="lg" variant="secondary" width="100%">
                      {option.title}
                    </Button>
                  </Link>
                </GridItem>
