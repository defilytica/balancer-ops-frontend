...
const HOOK_TYPE_MAP = {
  "stable-surge": {
    ...
    type: "STABLE_SURGE" as const,
    name: "StableSurge",
  },
  "mev-tax": {
    tag: "HOOKS_MEVCAPTURE",
    type: "MEV_TAX" as const,
    name: "MEV Capture",
  },
} as const;

type URLHookType = keyof typeof HOOK_TYPE_MAP;

export default function HooksTablePage() {
  ...
  const hookType =
    hookTypeParam && HOOK_TYPE_MAP[hookTypeParam]
      ? HOOK_TYPE_MAP[hookTypeParam]
      : HOOK_TYPE_MAP["stable-surge"];
  ...
  const networkOptionsWithAll = useMemo(() => {
    ...
    // For MEV Capture hook, restrict to Base and Optimism
    ...
    // For other hook types, include all networks
    return [...baseOptions, ...NETWORK_OPTIONS];
  }, [hookType.type]);
  ...
  useEffect(() => {
    if (hookTypeParam && !HOOK_TYPE_MAP[hookTypeParam]) {
